...
# ----------------- IMPORTAR FINANCEIRO VIA EXCEL -----------------
@app.route('/financeiro/importar', methods=['GET', 'POST'])
@login_required
    ...
    if request.method == 'POST':
        ...
        if arquivo.filename.endswith(('.xlsx', '.xls')):
            ...
            try:
                ...
                flash(f'Erro na leitura do arquivo: {e}')
                return redirect(url_for('importar_financeiro'))

        else:
            flash('Formato de arquivo inválido. Envie um arquivo Excel (.xlsx ou .xls)')
            return redirect(url_for('importar_financeiro'))
...
def exportar_financeiro():
    registros = Financeiro.query.order_by(Financeiro.data).all()

    if not registros:
        flash("Não há lançamentos financeiros para exportar.", "warning")
        return redirect(url_for('financeiro'))
